...
#define MAX_AUDIO_FRAME_SIZE 192000
	...
	void getFrameData(videoPB* vid, char* buf) {
	...
	int64_t getLastDTS(videoPB* vid) {
		return vid->last_dts;
	}

	int64_t getLastPTS(videoPB* vid) {
		return vid->last_pts;
	}

	int64_t getLastPktPTS(videoPB* vid) {
		return vid->last_pkt_pts;
	}

	int64_t getTimeBase(videoPB* vid) {
		return vid->time_base;
	}

	int64_t getDelay(videoPB* vid) {
		return vid->delay;
	}

	int getFrameLineSize(videoPB* vid) {
...
#define SDL_AUDIO_BUFFER_SIZE 2024
#define MAX_AUDIO_FRAME_SIZE 392000

	bool defSDL = false;

	int stopAudio(videoPB* v) {
