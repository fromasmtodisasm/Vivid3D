...
void CLImageProcessor::SetImageBuffer(unsigned char* buf) {
	...
	CLImgBuf = cl::Buffer(Application::Main->GetCLContext(),CL_MEM_READ_ONLY,(size_t)(ImgW * ImgH * bpp));
	CLOutput = cl::Buffer(Application::Main->GetCLContext(),CL_MEM_READ_WRITE  , (size_t)(ImgW * ImgH * bpp));
...
void CLImageProcessor::SetSource(const char* src) {
	...
	program = cl::Program(Application::Main->GetCLContext(), code);

	if (program.build({ Application::Main->GetCLDevice() }) != CL_SUCCESS) {
		std::cout << " Error building: " << program.getBuildInfo<CL_PROGRAM_BUILD_LOG>(Application::Main->GetCLDevice()) << "\n";
		exit(1);
	}

	CLQueue = cl::CommandQueue(Application::Main->GetCLContext(), Application::Main->GetCLDevice());
